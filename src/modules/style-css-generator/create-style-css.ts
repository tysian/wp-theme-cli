...
export const styleCssGenerator = async (options: StyleCssGeneratorOptions) => {
  ...
  try {
     ...
     * - [x] check if style.css exists in CWD
     * - [x] if there is an `--dont-overwrite` option, then throw Error
     * - [x] assume that first comment is a meta description
     * - [x] update first comment by new data generated from `package.json`
     * - [x] add `generated by wp-theme-cli + current date
     * - [x] save new style.css
     */

    const { validateSchema = false, dontOverwrite = false } = options;
